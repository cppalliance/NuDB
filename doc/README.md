...
2. Build Docker image. From the NuDB root folder:
```
sudo docker build -t nudb-docs doc/
```

## Setup project submodules

1. Open a shell in your NuDB root folder.
2. `git submodule init`
3. `git submodule update doc/docca`

## Do it

### Windows & MacOS

From the NuDB root folder:
```
cd doc
./makeqbk.sh && b2
```
...
From the NuDB root folder:
```
sudo docker run -v $PWD:/opt/nudb --rm nudb-docs
```
The output will be in `doc/html`.
