  ...
  <xsl:for-each select="
    ...
    <xsl:choose>
      <xsl:when test="@kind='class' or @kind='struct'">
        <xsl:if test="
            contains(compoundname, 'nudb::') and
...
<xsl:template name="strip-nudb-ns">
  <xsl:param name="name"/>
  <xsl:choose>
    <xsl:when test="contains($name, 'boost::system::is_error_code_enum')">
      <xsl:value-of select="$name"/>
    </xsl:when>
    <xsl:when test="contains($name, 'nudb::')">
      <xsl:value-of select="substring-after($name, 'nudb::')"/>
    </xsl:when>
    <xsl:when test="$name = 'nudb'">
      <xsl:text></xsl:text>
    </xsl:when>
    <xsl:otherwise>
      <xsl:value-of select="$name"/>
...
<xsl:template match="ref[@kindref='compound']" mode="markup">
  ...
  <xsl:choose>
    <xsl:when test="contains(@refid, 'nudb')">
      <xsl:variable name="dox-ref-id" select="@refid"/>
      <xsl:variable name="ref-name">
        <xsl:call-template name="strip-nudb-ns">
          <xsl:with-param name="name"
      ...
      <xsl:text>']|</xsl:text>
      -->
      <xsl:text>[link nudb.ref.</xsl:text>
      <xsl:value-of select="$ref-id"/>
      <xsl:text> `</xsl:text>
...
<xsl:template match="ref[@kindref='member']" mode="markup">
  ...
  <xsl:choose>
    <xsl:when test="contains(@refid, 'nudb') and count($memberdefs) &gt; 0">
...
<xsl:template name="header-requirements">
  ...
  <xsl:text>] &#xd;&#xd;</xsl:text>
  <xsl:choose>
    <xsl:when test="contains($file, 'nudb/core')">
      <xsl:text>['Convenience header: ][^nudb/core.hpp]</xsl:text>
    </xsl:when>
    <xsl:when test="contains($file, 'nudb/http')">
      <xsl:text>['Convenience header: ][^nudb/http.hpp]</xsl:text>
    </xsl:when>
    <xsl:when test="contains($file, 'nudb/websocket')">
      <xsl:text>['Convenience header: ][^nudb/websocket.hpp]</xsl:text>
    </xsl:when>
    <xsl:when test="contains($file, 'nudb/')">
    </xsl:when>
    <xsl:otherwise>
...
<xsl:template name="class-tables">
  ...
  <xsl:if test="count(sectiondef[@kind='related']/memberdef) &gt; 0">
    ...
    <xsl:for-each select="sectiondef[@kind='related']/memberdef" mode="class-table">
      ...
      <xsl:if test="$overload-position = 1">
      [
        [[link nudb.ref.<xsl:value-of select="$class-id"/>.<xsl:value-of select="$id"/>
...
<xsl:template match="param" mode="class-detail-template">
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
      <xsl:value-of select="type"/>
      <!--<xsl:value-of select="concat(' ``[link nudb.ref.', declname, ' ', declname, ']``')"/>-->
    </xsl:otherwise>
  </xsl:choose>
